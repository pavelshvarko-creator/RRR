...
export const App = () => {
  ...
  useEffect(() => {
    ...
    // .catch — если запрос по какой-то причине упал с ошибкой (например
    // ExtendScript-бандл ещё не успел зарегистрироваться к моменту вызова
    // сразу при монтировании панели), nameSetupNeeded раньше застревал в
    // начальном null навсегда: и кнопка "i" (=== false), и диалог первого
    // запуска (=== true) больше никогда не появлялись — единственный
    // доступ к гайду пропадал без следа. Считаем сбой равносильным
    // "имя ещё не задано" — тогда хотя бы диалог первого запуска покажется.
    evalTS("getSavedCreatorName")
      .then((saved) => setNameSetupNeeded(!saved))
      .catch(() => setNameSetupNeeded(true));
